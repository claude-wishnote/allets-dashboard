...
<%
    String docsUrl=(String)request.getAttribute("docsUrl");
%>
{}<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <script src="../jsonDoc/js/jquery-1.11.1.min.js"></script>
    <script src="../jsonDoc/js/bootstrap.min.js"></script>
    <script type="text/javascript" src="../jsonDoc/js/handlebars-1.0.0.beta.6.js"></script>
    <script type="text/javascript" src="../jsonDoc/js/jlinq.js"></script>
    <script type="text/javascript" src="../jsonDoc/js/prettify.js"></script>

    <!-- Le styles -->
    <link href="../jsonDoc/css/bootstrap.min.css" rel="stylesheet">
    <style type="text/css">
        body {
            padding-top: 60px;
            padding-bottom: 40px;
    ...
    <div class="container-fluid">
        ...
        <form class="navbar-form navbar-left col-md-8" role="search">
            <div class="form-group">
                <input id="jsondocfetch" type="text" class="form-control" style="width:350px" placeholder="Insert here the JSONDoc URL" value="<%=docsUrl%>" autocomplete="off">
                <input id="password" type="password" class="form-control" style="width:150px" placeholder="password" autocomplete="off">
            </div>
...
<script id="main" type="text/x-handlebars-template">
    <blockquote>
        <p style="text-transform: uppercase;">API info</p>
            ...
            <div id="_{{jsondocId}}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="{{jsondocId}}">
                <div class="panel-body">
                    <table class="table table-condensed table-bordered" style="table-layout: fixed;">
                        ...
                        <tr>
                            <td></td>
                            <td>Type: <code>{{this.jsondocType.type}}</code></td>
                        </tr>
                        ...
                        <tr>
                            <td colspan=2><code>{{bodyobject.jsondocType.type}}</code></td>
                        </tr>
                        {{/if}}
                        {{#if response}}
                        <tr>
                        ...
                        <tr>
                            <td colspan=2><code>{{response.jsondocType.type}}</code></td>
                        </tr>
                        {{/if}}
                        {{#if apierrors}}
                        <tr>
...
<script id="test" type="text/x-handlebars-template">
    ...
    <div class="row">
        <div class="col-md-12">
            <h4>Allets Authentication</h4>
            <div class="form-group" style="margin-bottom:5px;">
                <label for="x-allets-token">x-allets-token</label>
                <input class="form-control" type="text" id="x-allets-token" name="x-allets-token" placeholder="x-allets-token">
            </div>
        </div>
            ...
            <div class="tabbable" id="resInfo" style="display:none; margin-top: 20px;">
                ...
                <div class="tab-content" style="margin-top: 20px">
                    ...
                    <div class="tab-pane" id="tab2">
                        <h5 style="padding:0px">Response code</h5>
                        <pre id="responseStatus" class="prettyprint">
                        </pre>
                        <h5 style="padding:0px">Response headers</h5>
                        <pre id="responseHeaders" class="prettyprint">
                        </pre>
                    </div>
                    <div class="tab-pane" id="tab3">
                        <h5 style="padding:0px">Request URL</h5>
...
<script id="object" type="text/x-handlebars-template">
    <table class="table table-condensed table-striped table-bordered" style="table-layout: fixed;">
        ...
        <tr><td></td><td>Type: <code>{{jsondocType.type}}</code></td></tr>
        {{#if format}}
...
Handlebars.registerHelper('checkAuthType', function(value) {
    var result="None";
    if(value!="NONE") {
        result="Required";
    }
...
function checkURLExistence() {
    var jsondocfetch = $("#jsondocfetch").val();
    var password = $("#password").val();
    if(jsondocfetch.trim() == '') {
        ...
        if(password.trim() == '') {
            ...
            return fetchdoc(jsondocfetch, password);
        }
    }
}
...
function fetchdoc(jsondocurl,password) {
    $.ajax({
        url : jsondocurl+"?password="+password,
        type: 'POST',
        dataType: 'json',
                    ...
                    $('#content a[rel="method"]').each(function() {
                        $(this).click(function() {
                            ...
                            $("#testButton").click(function() {
                                ...
                                if(method.auth) {
                                    if(method.auth.type != "NONE") {
                                        headers["x-allets-token"] = $('#x-allets-token').val();
                                    }
                                }
                                ...
                                $("#queryparameters input").each(function() {
                                    //for more clean document path replace -> add param
                                    //tempReplacedPath = replacedPath.replace("{"+this.name+"}", $(this).val());
                                    //replacedPath = tempReplacedPath;
                                    tempReplacedPath = replacedPath+this.name+"="+$(this).val()+"&";
                                    replacedPath = tempReplacedPath;
                                });
