...
//        }
    ...
    @RequiresAuthentication
    @RequestMapping(value = "/v1.2" + "/monitors/handleHistory", method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
    public
    @ApiResponseObject
    void getMonitorHandleHistoryByUid(
            @ApiBodyObject Model model,
            @ApiParam(name = "monitorIds",
                    description = "monitorIds",
                    required = true,
                    paramType = ApiParamType.QUERY) @RequestParam(value = "monitorIds", required = false) String monitorIds,
            @ApiParam(name = "q",
                    description = "",
                    required = false,
        ...
        model.addAttribute(monitorFacade.findMonitorHandleHistory(monitorIds, q));
    }

    @RequiresAuthentication
    @RequestMapping(value = "/v1.2" + "/monitors/statistics", method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
    @ResponseBody
    public
    @ApiResponseObject
    ResponseEntity<byte[]> getStatistics(
            @ApiBodyObject Model model,
            @ApiParam(name = "q",
                    description = "",
                    required = false,
        ...
        HSSFWorkbook hssfWorkbook = monitorFacade.findStatistics(q);
        String title = hssfWorkbook.getSheet("statistics").getRow(0).getCell(0).getStringCellValue();
        response.setContentType("application/octet-stream");
        response.setHeader("name", title + ".xls");
        response.setHeader("Cache-Control", "must-revalidate, post-check=0, pre-check=0");
        response.setHeader("Pragma", "public");
        response.setDateHeader("Expires", 0);
        response.setHeader("Content-disposition", "attachment; filename=\"" + URLEncoder.encode(title + ".xls", "UTF-8") + "\"");
        hssfWorkbook.write(response.getOutputStream()); // 输出流控制workbook
